...
- [x] попробовать сгенерить данные для нейросетевого слотфиллера и обучить его (получилось пока неочень)
...
- [x] написать постпроцессор для выдачи ранкера (он работает так, что индексирует все товары для каждого сайта; нужно будет сгруппировать все товары из одного сайта в один результат и выдать уже список спецпредложений)
- [x] обернуть ранкер и запушить в мастер
- [x] добавить словарь для слотфиллинга и протестировать его
...
- [ ] написать программу для считывания всех напарсенных данных и индексации в эластик
