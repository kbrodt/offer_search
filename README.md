...
Появились данные по [*Спорт*](./data/sport) и [*Еда и продукты*](./data/food) по парсингу. Результатом парсинга является два файла `data.xls` и `meta.xls`  помещаются в папку с названием как сайт у спецпредложения. В `data.xls` хранится следующая информация:
...
Также написан первый прототип для слотфиллинга и проверен на примерах, где товаром является *велосипед*. Для проверки на других примерах нужны названия всех имеющихся товаров и прочие атрибуты. Появиться после парсинга сайтов.
