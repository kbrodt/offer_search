			...
			$.ajax({
				...
				success: function(data){
					...
					for(var i=0; i < my_json.length; i++)
					{
						...
						$('#all_items_block').append('<div class="col-xs-12 accordion" style="cursor: pointer;"><a class="item-block"><header><img src="/static/assets/img/Tinkoff-bank.png" alt=""><div class="hgroup" style="display: inline;"><h4 href="' + my_json[i]['offer'].web +'" class="go_to_href">' + my_json[i]['offer'].offer +'</h4><h5>' + my_json[i]['offer'].advert_text + '</h5></div><div class="header-meta" style="position: absolute; top: 14px; right: 37px;"><span class="location">КБ '+ my_json[i]['offer'].cashback +'%</span><span class="label label-success">' + my_json[i]['offer'].period + ' месяцев</span></div></header></a></div><div class="panel" style="margin-left: 40px; margin-right: 40px;">'+ all_products +'</div>');
					}
					var acc = $(".accordion");
					var i;
